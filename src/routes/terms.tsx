...
export const Route = createFileRoute("/terms")({
  component: Terms,
  head: () => ({ meta: [{ title: "Terms & Conditions — AuraTrad.Ai" }] }),
});

function Terms() {
  return (
    <>
      <PageHero eyebrow="Legal" title={<>Terms & <span className="gradient-text">Conditions</span></>} subtitle="Please read these terms carefully before using AuraTrad.Ai." />
      <Section>
        <div className="prose prose-invert max-w-3xl mx-auto text-muted-foreground space-y-5 text-sm leading-relaxed">
          <p>By accessing AuraTrad.Ai, you agree to these Terms. Crypto investments carry risk including potential loss of capital. Past performance does not guarantee future results.</p>
          ...
          <p>AuraTrad.Ai is provided "as is". To the maximum extent permitted by law, we disclaim warranties for losses arising from market volatility.</p>
        </div>
      </Section>
    </>
  );
